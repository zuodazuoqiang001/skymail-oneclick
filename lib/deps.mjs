...
export function createPnpmRunner(pnpmCjs, registry) {
  return {
    registry,
    bin: pnpmCjs,
    run(args, opts) {
      ...
      return run(process.execPath, [pnpmCjs, ...args], Object.assign({}, opts, { env }));
    },
  };
}

export async function ensurePnpm(root, onEvent) {
  ...
  if (fs.existsSync(js)) {
    try {
      await run(process.execPath, [js, '--version'], { timeout: 15000 });
      emit(onEvent, { step: 'install', status: 'ok', message: 'Reuse local pnpm at work/tools' });
      return createPnpmRunner(js, REGISTRIES[0]);
    } catch (e) {
  ...
  if (!(await commandExists('npm'))) {
    throw new Error('需要 Node.js 自带的 npm。请先安装 Node.js 20+ 后再部署。');
  }
  ...
  for (const registry of REGISTRIES) {
    ...
    try {
      ...
      if (fs.existsSync(js)) {
        ...
        return createPnpmRunner(js, registry);
      }

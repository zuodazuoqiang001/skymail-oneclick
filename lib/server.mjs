...
import { cleanupOrphans, createCf, findNamedResources, getCatchAllRule, isCfAuthError, listAccounts, listZones, resolveNamePlan, verifyToken } from './cf.mjs';
...
import { currentNodeInfo } from './node.mjs';

export const APP_VERSION = '0.3';
export const WIZARD_VERSION = 13;
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && pathname === '/api/health') {
        return send(res, 200, { ok: true, name: 'skymail-oneclick', appVersion: APP_VERSION, version: WIZARD_VERSION, features: ['cleanup', 'mirrors', 'deps', 'node22'], node: currentNodeInfo() }, { 'Content-Type': 'application/json; charset=utf-8' });
      }
      ...
      if (req.method === 'POST' && pathname === '/api/session') {
        ...
        let emailRules = null;
        if (zones && zones[0] && zones[0].id) {
          try {
            await getCatchAllRule(cf, zones[0].id);
            emailRules = { ok: true, zoneId: zones[0].id, zoneName: zones[0].name };
          } catch (e) {
            emailRules = {
              ok: false,
              zoneId: zones[0].id,
        ...
        return send(
          res,
          200,
          { verify, accounts, zones, emailRules },

...
export async function ensureRepo(workDir, onEvent) {
  ...
  if (hasGit && looksLikeRepo(repoDir) && fs.existsSync(path.join(repoDir, '.git'))) {
    emit(onEvent, { step: 'clone', status: 'run', message: 'Pull latest cloud-mail from GitHub' });
  ...
  if (fs.existsSync(repoDir)) {
    emit(onEvent, { step: 'clone', status: 'log', message: 'remove local source, clone latest from GitHub' });
    rmDir(repoDir);
  }
